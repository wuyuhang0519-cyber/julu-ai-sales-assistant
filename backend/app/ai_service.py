...
from .rag_service import knowledge_base,detect_language

PROMPT_VERSION="sales-qualification-v3.0.0-rag-multilingual"
...
def demo_result(lead,profile,message:str|None,first=False)->AIResult:
    ...
    lang=detect_language(text or lead.initial_requirement or lead.country)
    if lang=="en-US":
        if refuse:reply="Understood. I will stop automated sales follow-up. Contact JULU AI again whenever you need help."
        elif pricing:reply="A binding price, contract, or performance commitment requires a sales specialist. I have routed this conversation for human review."
        elif offer:reply="Your target market, timeline, and business needs are now clear. The next best step is a 45-minute discovery meeting with a JULU AI specialist."
        else:reply=f"Thanks, that helps. The highest-priority missing detail is {question}. Could you share a short answer?"
    elif lang=="es-ES":
        if refuse:reply="Entendido. Detendré el seguimiento comercial automático."
        elif pricing:reply="Un precio, contrato o compromiso de resultados requiere revisión de un asesor. He derivado la conversación a una persona."
        elif offer:reply="La información principal ya está clara. El siguiente paso recomendado es una reunión de diagnóstico de 45 minutos."
        else:reply=f"Gracias. El dato pendiente más importante es {question}. ¿Podrías compartirlo brevemente?"
    rag_query=" ".join(filter(None,[lead.industry,lead.interested_service,lead.initial_requirement,text]))
    _,refs=knowledge_base.context(rag_query)
    return AIResult(reply=reply,extracted_facts=updates,updated_profile=updates,missing_fields=missing,lead_score=score,intent=cast(Any,_intent(score)),score_breakdown=breakdown,score_reason="；".join(f"{k} {v['score']}/{v['max_score']}" for k,v in breakdown.items()),recommended_service="全球 AI 搜索优化 / GEO 落地页" if "国内" not in blob else "国内 AI 搜索优化",next_action=action,suggested_status=suggested,should_offer_meeting=offer,should_handoff=handoff,handoff_reason="涉及价格、合同或效果承诺" if pricing else None,conversation_summary=f"{lead.company} 正在评估 {lead.interested_service}；已识别 {len(facts)} 项需求信息。",question_reason=f"{question}是当前影响资格判断的最高优先级缺失信息",knowledge_refs=refs,fact_sources={k:[] for k in updates})

def _prompt(lead,profile,messages,first):
    latest=messages[-1].content if messages else (lead.initial_requirement or "")
    query=" ".join(filter(None,[lead.industry,lead.country,lead.interested_service,lead.initial_requirement,latest]))
    kb,refs=knowledge_base.context(query)
    language=detect_language(latest or lead.initial_requirement or lead.country)
    return f"""你是聚路 AI 销售资格判断助手。Prompt版本:{PROMPT_VERSION}。只依据检索知识片段与客户证据，严禁虚构价格、案例、排名或合同承诺。每轮最多两个问题，不重复已知信息。首次接待必须个性化。用客户语言 {language} 回复。输出事实来源消息ID、问题原因和知识引用；knowledge_refs 只能使用本轮允许引用 {refs}。只输出符合JSON Schema的对象。
...
def run_ai(lead,profile,messages,first=False)->AIExecution:
    ...
    client=OpenAI(api_key=api_key,base_url=base_url,timeout=settings.ai_timeout_seconds,max_retries=settings.ai_max_retries);schema=AIResult.model_json_schema();prompt,retrieved_refs=_prompt(lead,profile,messages,first);raw="";repair=0
    try:
        for attempt in range(2):
            ...
            try:
                result=AIResult.model_validate_json(raw);result.knowledge_refs=[x for x in result.knowledge_refs if x in retrieved_refs] or retrieved_refs;lat=int((time.perf_counter()-start)*1000)
